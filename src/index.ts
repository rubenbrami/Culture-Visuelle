...
let selectedArticle: ArticleDescriptionAndPosition = null
...
window.addEventListener("mousedown", (e: MouseEvent) => {
    prevMouseX = e.x
    prevMouseY = e.y
    let mX = (e.x - bg.translationX) / bg.myScale / window.innerWidth
    let mY = (e.y - bg.translationY) / bg.myScale / window.innerHeight
    articles.forEach((article, i) => {
        let d = (mX-article.x)*(mX-article.x) + (mY-article.y)*(mY-article.y)
        if (d < 0.0017) {
            if (e.button == 0)
                selectedArticle = article
            if (e.button == 1) {
                if (edgeStart != null) {
                    edges.push([edgeStart, i])
                    edgeStart = null
                    bg.nodesX1 = []
                    bg.nodesY1 = []
                    bg.nodesX2 = []
    ...
    if (selectedArticle == null && edgeStart == null)
        isDragging = true
    if (e.button == 2) {
        let str = ""
        articles.forEach(article => {
            str += `x: ${article.x}, y: ${article.y}\n`
        })
        console.log(str)
        console.log(edges)
    }
...
window.addEventListener("mousemove", (e: MouseEvent) => {
    ...
    if (selectedArticle) {
        selectedArticle.x += (e.x - prevMouseX) / window.innerWidth  / bg.myScale
        selectedArticle.y += (e.y - prevMouseY) / window.innerHeight / bg.myScale
        prevMouseX = e.x
        prevMouseY = e.y
        draw()
        bg.nodesX1 = []
